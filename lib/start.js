...
\t${optional('--repo=all')} \t\t  for all repositories (${enhance('default')})`],
        ['', 'proxy[=proxy]', `\tSet up proxy to cat trunk via svn:
\t${optional('--proxy=socks5://127.0.0.1:1080')}
\t${optional('--proxy=http://127.0.0.1:1080')}`],
    ...
    Object.entries(data).sort((a, b) => b[1] - a[1]).forEach(([key, count]) => {
        ...
        table.push({
            [key]: [
                ...
                optional(`https://img.shields.io/badge/%20%20${encodeURIComponent(key)}-%20%20%20%20${texture}-${(require('./linguist')[key].color || '#ededed').slice(1)}.svg`),
            ],
        });
    });
