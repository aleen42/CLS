...
/**
 * [progressBar: a module to show progress]
 * @type {Object}
 */
...
module.exports = {
	...
	getJSON: function (requestUri, callback) {
		request({
			...
			if (typeof(response) == 'undefined') {
				callback(body, '[Error: please check network!]');
				return;
			}
	...
	statistic: function (uriArr, callback, data, index, line) {
		...
		if (line) {
			_this.getJSON(uriArr[index].languages_url, function (body, error) {
				const obj = JSON.parse(body);
				...
				switch (process.platform) {
					case 'darwin':
						...
						for (var lang in obj) {
							...
							larr.push(_this.fileSuffix[lang].join(' -e '));
						}

						farr = execSync('svn ls -R "' + uriArr[index].clone_url + '/trunk/" | grep -e ' + larr.join(' -e ') + ' | xargs -n1 echo ' + uriArr[index].clone_url + '/trunk/ | awk "{print $1$2}"').toString();

						/** [if: prevent from occuring situation: svn error] */
						if (farr == '') {
							farr = execSync('svn ls -R "' + uriArr[index].svn_url + '/trunk/" | grep -e ' + larr.join(' -e ') + ' | xargs -n1 echo ' + uriArr[index].svn_url + '/trunk/ | awk "{print $1$2}"').toString();
						}
						...
						farr = farr.replaceAll('/trunk/ ', '/trunk/');
					...
					case 'linux':
						...
						/** @deprecated */
						// var cmd = './lib/list ' + uriArr[index].clone_url;
						// farr = execSync(cmd + ' | egrep "' + larr.join('|') + '" | xargs -n1 -i echo ' + uriArr[index].clone_url + '/{} | awk "{print $1$2}"').toString();

						var cmd = 'svn ls -R "' + uriArr[index].clone_url + '/trunk/"';
						farr = execSync(cmd + ' | egrep "' + larr.join('|') + '" | xargs -n1 -i echo ' + uriArr[index].clone_url + '/trunk/{} | awk "{print $1$2}"').toString();
						
						/** [if: prevent from occuring situation: svn error] */
						if (farr == '') {
							/** @deprecated */
							cmd = './lib/list ' + uriArr[index].svn_url;
							farr = execSync(cmd + ' | egrep "' + larr.join('|') + '" | xargs -n1 -i echo ' + uriArr[index].svn_url + '/{} | awk "{print $1$2}"').toString();
							
							// cmd = 'svn ls -R "' + uriArr[index].svn_url + '/trunk/"';
							// farr = execSync(cmd + ' | egrep "' + larr.join('|') + '" | xargs -n1 -i echo ' + uriArr[index].svn_url + '/trunk/{} | awk "{print $1$2}"').toString();
						}
						break;
					default:
						break;
				...
				if (farr == '') {
					console.log('[Error: error to access repositories]');
					return;
				}
			...
			_this.getJSON(uriArr[index], function (body, error) {
				/** exceptions */
				error = error || '';
