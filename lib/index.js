...
module.exports = {
	 ...
	 * @return {[type]}          [description]
	 */
	listRepo: function (userName, callback, data, page) {
		const perPage = 100;
		const requestUri = 'https://api.github.com/users/' + userName + '/repos?per_page=100&page=' + page + '&access_token=f2efb655cd27fc92d513b9b09d7286b3ee1ee1ff';
		const _this = this;
		const charArr = ['/', '-', '\\', '-'];
		...
		this.getJSON(requestUri, function (body, error) {
			...
			if (obj.length == 0) {
				...
				_this.listRepo(userName, callback, data, ++page);
			}
		});
	},
	...
	calculate: function (userName, options) {
		...
		this.listRepo(userName, function(data) {
			...
			for (var item in data) {
				...
				langUri.push(data[item].languages_url + '?access_token=f2efb655cd27fc92d513b9b09d7286b3ee1ee1ff');
			}
