 ...
 * @type {object}
 */
const linguist = require('language-map');
/**
 * stdio stream
 * @type {process.stdout|{}}
 */
...
module.exports = {
     ...
     * @param {number}   [batchNum]     batching task number
     * @param {function} callback       callback table
     */
    calculate({user: owner, filter = 'original', accurate = false, line: showLine = false, token: accessToken = '', repo = 'all', batch: batchNum = 6}, callback) {
        if (!owner) {
        ...
        const _handler = repos => {
            ...
            (statistic = (repos, callback, showLine) => {
                ...
                const _pattern = lang => linguist[lang].extensions.map(pattern => `${pattern.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&')}$`).join('|');

                if (showLine) {
                    repos
                        .splice(0, index > 0 ? 1 : batch)
                        .forEach(({name, languages_url: languages, clone_url: clone, svn_url: svn}) => {
                            current.unshift(name);
                            load(`${languages}?access_token=${token}`, result => {
                                ...
                                switch (process.platform) {
                                    ...
                                    case 'win32':
                                    case 'linux':
                                        Promise.all([clone, svn]
                                            .map(url => new Promise(resolve => {
                                                exec(`svn --non-interactive --trust-server-cert ls -R "${url}/trunk/"`, {
                                                        encoding: 'utf8',
                                                        maxBuffer: 5000 * 1024, /** 200 * 1024 by default */
                                                    }, (err, stdout) => err ? resolve([]) : resolve(extract(stdout)
                                                        .filter(i => RegExp(repoLanguages.map(_pattern).join('|')).test(i))
                                                        .map(i => `${url}/trunk/${i}`))
                                                );
                                            }))
                                        ).then(([cloneFiles, svnFiles]) => {
                                            files = [].concat(cloneFiles).concat(svnFiles);
                                            if (!files.length) {
                                            ...
                                            Promise.all(repoLanguages.map(lang => Promise.all(files
                                                ...
                                                .map(path => new Promise((resolve, reject) => {
                                                    exec(`svn --non-interactive --trust-server-cert cat "${path}"`, {
                                                        encoding: 'utf8',
                                                        maxBuffer: 5000 * 1024, /** 200 * 1024 by default */
                                                    }, (err, stdout) => err
                                                        ? resolve(0)
...
function listRepo(owner, callback, data = [], page = 1) {
    ...
    load(`https://api.github.com/users/${owner}/repos?per_page=100&page=${page}&access_token=${token}`, result => {
        loader.loaded();
        if (!result) return;
...
function getRepo(owner, repo, callback) {
    ...
    load(`https://api.github.com/repos/${owner}/${repo}?access_token=${token}`, result => {
        loader.loaded();
        if (!result) return;
        callback([result]);
    });
